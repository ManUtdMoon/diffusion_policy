...
from zprl.env_runner.robomimic_delay_image_runner import RobomimicDelayImageRunner
...
def main(checkpoint, output_dir, action_delay_steps, server_addr, timeout_ms):
    ...
    runner_cfg = cfg.online_task.env_runner
    runner_cfg.n_train = 0
    runner_cfg.n_train_vis = 0
    runner_cfg.n_test = 100
    runner_cfg.n_test_vis = 0
    runner_cfg.test_start_seed = 100_000
    runner_cfg.n_envs = 50

    task_name = cfg.online_task.task_name
    dataset_type = cfg.online_task.dataset_type
    dataset_filename = os.path.basename(runner_cfg.dataset_path)
    dataset_path = os.path.join(DATASET_ROOT, task_name, dataset_type, dataset_filename)
    runner_cfg.dataset_path = dataset_path

    shape_meta = OmegaConf.to_container(runner_cfg.shape_meta, resolve=True)
    env_runner = RobomimicDelayImageRunner(
        ...
        n_obs_steps=OmegaConf.select(cfg, 'n_obs_steps', default=runner_cfg.n_obs_steps),
        n_action_steps=OmegaConf.select(cfg, 'n_action_steps', default=runner_cfg.n_action_steps),
