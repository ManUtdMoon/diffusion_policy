...
class TrainOnlineReactiveRobomimicWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        Tr = cfg.n_rl_steps
        assert Ta > 0 and Tr > 0 and Ta % Tr == 0, f"n_action_steps({Ta}) must be divisible by n_rl_steps({Tr})"
        n_segments = Ta // Tr
        do = self.base_policy.obs_feature_dim
        Do_base = To * do
        Do_aug = Do_base + d_mask
        da = cfg.shape_meta.action.shape[0]
        Da = Tr * da  # residual action segment dim

        self.res_policy: ResiduePolicy = hydra.utils.instantiate(
            cfg.res_policy, obs_dim=Do_aug, action_dim=Da)
        print(f"Residue policy with Do={Do_aug}, Da={Da}, Ta={Ta}, Tr={Tr}, gamma={self.res_policy.gamma}")
        ...
        dummy_obs_space = gymnasium.spaces.Box(
            ...
            shape=(Do_aug + Da,), dtype=np.float32
        )
        ...
        n_step = getattr(cfg.training, 'n_step', 1)
        assert isinstance(n_step, int) and n_step > 0
        rb = ReactiveNStepReplayBuffer(
            cfg.training.buffer_size,
            dummy_obs_space,
            dummy_buf_action_space,
            device=device,
            n_envs=cfg.training.n_envs,
            handle_timeout_termination=bootstrap_at_done == 'truncated',
            n_steps=n_step,
            gamma=self.res_policy.gamma,
            base_action_dim=Da,
        )
        ...
        @torch.no_grad()
        def encode_obs(obs_seq, predict_action=True):
            obs = dict_apply(
                obs_seq, lambda x: torch.from_numpy(x).to(device=device))
            return sum_policy.encode_obs(obs, predict_action=predict_action)
        ...
        base_cache = base_dict['naction'].detach()
        assert_shape(base_cache, (n_envs, Ta, da))
        seg_idx = np.zeros(n_envs, dtype=np.int64)
        ...
        with JsonLogger(log_path) as logger:
            ...
            while self.global_step < n_steps:
                ...
                for _ in tqdm.tqdm(
                    ...
                    base_naction = torch.stack(
                        [
                            base_cache[i, idx * Tr:(idx + 1) * Tr]
                            for i, idx in enumerate(seg_idx)
                        ],
                        dim=0
                    )
                    assert_shape(base_naction, (n_envs, Tr, da))
                    base_flat = base_naction.flatten(start_dim=1).cpu().numpy()  # (B, Tr*da)
                    ...
                    next_idx = seg_idx + 1
                    boundary = next_idx == n_segments
                    mid_timeout_next_base = dict()
                    for i in timeouts:
                        if not boundary[i]:
                            j = next_idx[i]
                            mid_timeout_next_base[i] = base_cache[
                                i, j * Tr:(j + 1) * Tr].clone()

                    _, next_obs_emb = encode_obs(
                        next_obs_seq, predict_action=False)
                    refresh = np.flatnonzero(np.logical_or(boundary, dones))
                    if len(refresh) > 0:
                        new_dict, new_obs_emb = encode_obs(
                            dict_apply(next_obs_seq, lambda x: x[refresh]))
                        new_base = new_dict['naction'].detach()
                        assert_shape(new_base, (len(refresh), Ta, da))
                        refresh_t = torch.as_tensor(refresh, device=device)
                        base_cache[refresh_t] = new_base
                        next_obs_emb[refresh_t] = new_obs_emb
                        next_idx[refresh] = 0

                    seg_idx = next_idx
                    next_base = torch.stack(
                        [
                            base_cache[i, j * Tr:(j + 1) * Tr]
                            for i, j in enumerate(seg_idx)
                        ],
                        dim=0
                    )
                    assert_shape(next_base, (n_envs, Tr, da))

                    ## prepare transitions for rb
                    rb_next_obs_emb = next_obs_emb
                    rb_next_base = next_base
                    if timeouts:
                        rb_next_obs_emb = next_obs_emb.clone()
                        rb_next_base = next_base.clone()
                        terminal_obs = {
                            key: np.stack([
                                infos[i]['terminal_observation'][key]
                                for i in timeouts
                            ])
                            for key in next_obs_seq
                        }
                        # Batch all timeouts; mid-chunk plans are overwritten below.
                        terminal_dict, terminal_emb = encode_obs(terminal_obs)
                        timeout_t = torch.as_tensor(timeouts, device=device)
                        rb_next_obs_emb[timeout_t] = terminal_emb
                        rb_next_base[timeout_t] = terminal_dict['naction'][:, :Tr].detach()
                        for i, old_base in mid_timeout_next_base.items():
                            rb_next_base[i] = old_base

                    obs_to_save = np.concatenate(
                        [obs_emb_tensor.cpu().numpy(), base_flat], axis=-1)
                    next_obs_to_save = np.concatenate(
                        [
                            rb_next_obs_emb.cpu().numpy(),
                            rb_next_base.flatten(start_dim=1).cpu().numpy()
                        ], axis=-1)

                    rb.add(
                        obs=obs_to_save,
                        next_obs=next_obs_to_save,
...
def main(cfg):
    workspace = TrainOnlineReactiveRobomimicWorkspace(cfg)
    workspace.run()
