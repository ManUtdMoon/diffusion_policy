...
import zprl.model.common.tensor_util as tu
...
class CropRandomizerV3(nn.Module):
    ...
    def forward_in(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        n = x.shape[0]  # B*T (or just B if 4D)

        # Determine how many independent crops to sample
        if inputs.ndim == 5:
        ...
        if self.training or self.force_random_crop:
            ...
            # Sample B independent shifts, then repeat each T times
            shift_h = torch.randint(
                0, max_h + 1, size=(B, 1, 1, 1), device=x.device, dtype=x.dtype
            ) * (2.0 / H)
            shift_w = torch.randint(
                0, max_w + 1, size=(B, 1, 1, 1), device=x.device, dtype=x.dtype
            ) * (2.0 / W)
            shift = torch.cat([shift_w, shift_h], dim=-1)  # [B, 1, 1, 2]
            # [B, 1, 1, 2] -> [B, T, 1, 1, 2] -> [B*T, 1, 1, 2]
