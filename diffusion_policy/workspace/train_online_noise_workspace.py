...
from diffusion_policy.policy.noise_policy import NoisePolicy, SumPolicy
        ...
        with JsonLogger(log_path) as logger:
            ...
            while self.global_step < n_steps:
                ...
                for _ in tqdm.tqdm(
                    ...
                    with torch.no_grad():
                        next_obs_emb_tensor = self.base_policy.encode_obs(next_obs_seq_tensor)

                    if cfg.training.bootstrap_at_done == 'never':
                        next_obs_emb_store = next_obs_emb_tensor.detach().cpu().numpy()
                        ...
                        with torch.no_grad():
                            ...
                            next_obs_emb_store = real_next_obs_emb.detach().cpu().numpy()

                    rb.add(
                        obs=obs_emb_tensor.detach().cpu().numpy(),
                        next_obs=next_obs_emb_store,
