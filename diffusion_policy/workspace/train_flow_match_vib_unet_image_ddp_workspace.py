...
def _mean_metric_dict(accelerator: Accelerator, metric_dict, device: torch.device):
    result = dict()
    for key, value in metric_dict.items():
        result[key] = _mean_scalar(accelerator, value, device)
    return result
...
class TrainFlowMatchVibUnetImageDDPWorkspace(BaseWorkspace):
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}",
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        info_cpu = _mean_metric_dict(accelerator, info, device)
...
def main(cfg):
    workspace = TrainFlowMatchVibUnetImageDDPWorkspace(cfg)
    workspace.run()
