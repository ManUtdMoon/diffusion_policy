...
class RemoteImagePolicy:
    ...
    def predict_action(
        ...
        payload = {"obs": dict_apply(obs_dict, _to_cpu)}
        ...
        self.predict_latencies_sec.append(time.perf_counter() - t0)
    ...
    def _request(self, request_type: str, payload: dict):
        ...
        try:
            ...
            raise TimeoutError(
                ...
                f"while handling {request_type}"
            ) from e

        response = dill.loads(raw_response)
        if response.get("request_id") != request_id:
            raise RuntimeError(
                "Policy server returned mismatched request_id: "
