...
class BoxRunner(BaseImageRunner):
    def __init__(self,
        output_dir,
        eval_episodes=30,
        max_steps=400,
    ...
    def run(self, policy: FlowMatchVibUnetImagePolicy): 
        ...
        while completed_episodes < self.eval_episodes:
            ...
            while not episode_done:
                ...
                obs_dict_input['global'] = (obs['global']).astype(np.float32)
                obs_dict_input['wrist_0'] = (obs['wrist_0']).astype(np.float32)
                obs_dict_input['wrist_1'] = (obs['wrist_1']).astype(np.float32)
